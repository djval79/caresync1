...
import { X, User, Briefcase, Clock, ShieldCheck } from 'lucide-react';
import { StaffRole } from '../types';

interface AddStaffModalProps {
  onClose: () => void;
  onAdd: (staffData: any) => void;
}

const AddStaffModal: React.FC<AddStaffModalProps> = ({ onClose, onAdd }) => {
  const [formData, setFormData] = useState({
    name: '',
    role: StaffRole.CARER,
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform animate-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center">
              <Briefcase size={14} className="mr-2" /> Professional Role
            </label>
            <select
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center">
              <Clock size={14} className="mr-2" /> Weekly Contracted Hours
            </label>
            <input
              required
              type="number"
              ...
              onChange={e => setFormData({...formData, contractedHours: parseFloat(e.target.value)})}
            />
          </div>
